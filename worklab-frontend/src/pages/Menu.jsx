...
const navigation = [
  ...
  { name: "Vincular Exame", to: "/vincular_exame" },
      ...
      <div className="flex justify-center items-center flex-wrap gap-5">
        {navigation.map((item) => (
          <NavLink
            key={item.name}
            to={item.to}
            className={`text-white font-medium cursor-pointer text-[20px] hover:bg-primary bg-[#1B90C6] w-[160px] h-[160px] rounded-r-sm flex items-center justify-center`
            }
          >
           {item.name}
          </NavLink>
